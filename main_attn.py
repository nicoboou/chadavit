...
def main(cfg: DictConfig):
    ...
    # load custom pretrained weights
        ...
        if cfg.channels_strategy == "one_channel":
            ...
            backbone = VisionTransformerAttnViz(
                patch_size=16,
                embed_dim=192,
                depth=12,
                num_heads=3,
            ...
            backbone = modify_first_layer(
                backbone=backbone, cfg=cfg, pretrained=pretrained
            )
            state = torch.load(ckpt_path, map_location="cpu")["state_dict"]
    ...
    w, h = (
        img.shape[1] - img.shape[1] % cfg.patch_size,
        img.shape[2] - img.shape[2] % cfg.patch_size,
    )
    img = img[:, :w, :h].unsqueeze(0)

    w_featmap = img.shape[-2] // cfg.patch_size
    ...
    if cfg.threshold is not None:
        ...
        th_attn = (
            nn.functional.interpolate(
                th_attn.unsqueeze(0), scale_factor=cfg.patch_size, mode="nearest"
            )[0]
            .cpu()
            .numpy()
        )
    ...
    attentions = (
        nn.functional.interpolate(
            attentions.unsqueeze(0), scale_factor=cfg.patch_size, mode="nearest"
        )[0]
        .cpu()
        .numpy()
    )
    ...
    torchvision.utils.save_image(
        ...
        os.path.join(cfg.output_dir, "img.png"),
    )
    for j in range(nh):
    ...
    if cfg.threshold is not None:
        ...
        for j in range(nh):
            display_instances(
                ...
                fname=os.path.join(
                    cfg.output_dir,
                    "mask_th" + str(cfg.threshold) + "_head" + str(j) + ".png",
                ),
                blur=False,
            )
